...
static char		*ft_vec_pop(t_vec *v)
{
	...
	if (!(v->data))
		return (NULL);
	temp_tail = ft_strchr(DATA, '\n');
	v->data_tail = temp_tail;
	if (temp_tail)
	{
		*temp_tail = '\0';
		if (*(temp_tail + 1))
			v->data_tail = ++temp_tail;
		if ((output = ft_strdup(DATA)))
		{
			ft_strclr(DATA);
			ft_memmove(v->data, v->data_tail, ft_strlen(DATA_TAIL) + 1);
		}
	}
	else	if (ft_strlen(DATA) && (output = ft_strdup(DATA)))
	{
		ft_strclr(DATA);
		ft_memdel(&(v->data));
	}
